...
export class PercentageCalculator {
  ...
  result = computed(() => {
    ...
    switch (this.mode()) {
      case 'percent-of': return (xVal / 100) * yVal;
      case 'what-percent': return yVal > 0 ? (xVal / yVal) * 100 : 0;
      case 'change': return !xVal ? 0 : ((yVal - xVal) / xVal) * 100;
      default: return 0;
    }
  });
  ...
  setMode(mode: CalcMode): void { this.mode.set(mode); this.x.set(0); this.y.set(0); }
  onXChange(value: number): void { this.x.set(value); }
  onYChange(value: number): void { this.y.set(value); }
}