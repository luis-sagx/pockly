...
export class ImageResize {
  ...
  onWidthChange(val: number) {
    ...
    if (this.keepAspectRatio() && this.naturalWidth && this.naturalHeight)
      this.height.set(Math.round(val * (this.naturalHeight / this.naturalWidth)));
  }
  onHeightChange(val: number) {
    this.height.set(val);
    if (this.keepAspectRatio() && this.naturalWidth && this.naturalHeight)
      this.width.set(Math.round(val * (this.naturalWidth / this.naturalHeight)));
  }
  ...
  async process() {
    ...
    try {
      ...
      this.resultFileName.set(
        this.buildName(
          `${this.width()}x${this.height()}.${this.extensionFromFormat(this.outputFormat())}`,
        ),
      );
